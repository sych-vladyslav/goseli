use goseli_core::{
    dto::{CreateProductRequest, ProductListQuery, UpdateProductRequest},
...
pub async fn create_product(
    ...
    let status = req.status.as_ref().map(|s| s.to_string()).unwrap_or_else(|| "draft".to_string());
...
pub async fn update_product(
    ...
    let status = req.status.as_ref().map(|s| s.to_string()).unwrap_or(current.status.to_string());
